...
class AnsiAware
{
    ...
    /**
     * Calculate the display width of a plain string (no ANSI codes).
     */
    public static function stringWidth(string $string): int
    {
        if ($string === '') {
            return 0;
        }
        ...
        // Use grapheme_extract for proper Unicode grapheme iteration
        $width = 0;
        $offset = 0;
        $len = strlen($string);

        while ($offset < $len) {
            $grapheme = grapheme_extract($string, 1, GRAPHEME_EXTR_COUNT, $offset, $next);
    ...
    public static function wordwrap($string, $width = 75, $break = PHP_EOL, $cut = false): string
    {
        ...
        foreach ($parts as $part) {
            ...
            foreach ($wordsOrChars as $wordOrChar) {
                $length = self::stringWidth($wordOrChar);
    ...
    /**
     * Pad a string to a given length, accounting for ANSI codes.
     * If the string is longer than width, it will be truncated.
     */
    public static function pad(string $text, int $width, string $pad = ' ', int $type = STR_PAD_RIGHT): string
    {
        $visibleLength = self::mb_strlen($text);

        // Truncate if too long
        if ($visibleLength > $width) {
            return self::substr($text, 0, $width);
        }
        ...
        $padString = str_repeat($pad, $padding);

        return match ($type) {
            STR_PAD_LEFT => $padString.$text,
